...
for wire_diameter in np.arange(0.5, 0.6, 0.1): 
  for density in np.arange(n_initial,n_final,nstep):
      ...
      open(res_file,"a").write("%s %s %s %s %s %s %s %s %s %s %s %s %s %s %s %s %s %s %s %s %s %s\n" %(density,AF,transmittance,resAvg,resStd,junc_avg,R_junc,rho0,wire_diameter,wire_length,box_length,samples,nstep,n_initial,n_final,tol,distl,lower_l,upper_l,sigmal,percentage_chance,round(time.clock() - start_time, 5)))
