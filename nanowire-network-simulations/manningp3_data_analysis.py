...
for line in data:
    ...
    if i>0:
        ...
        if dens<d_prev:
             ...
             calctime.append([])
             #parameters below are constant for each density loop
             Rjcn.append(words[6])
             rho0.append(words[7])
             rho=float(words[7])
             ...
             #calculate nanotube resistance
             A0=math.pi*((diam*0.001)/2)**2
             wireres=leng*rho/A0
             wireres=round(wireres,4)
             nanotube_res.append(wireres)
...
#*******************************************
### Set varied parameter (e.g. wire length, wire diameter, box length) and labels for legend, axis, and plot title ###
#makes things easier when varying different parameters for different data sets

data_label=[]
varied_param=[]
for i in np.arange(0,j+1,1):
    data_label.append('wire length=%s um, Rtube= %s \u03A9,  Rjcn= %s \u03A9' %(wire_length[i],nanotube_res[i],Rjcn[i]))
    varied_param.append(wire_length[i])

print(data_label)
varied_param_axis='Wire Length (um)'
varied_param_title='Wire Length'

#************************************

#T vs Rs functions to  fit
...
#may need to adjust if additional colors necessary 
colors = ['b', 'g', 'r', 'c', 'm', 'y', 'k']

Rs_T95=[]
T95=[]

#T vs Rs fit and plot
for i in np.arange(0,j+1,1):
    ...
    #find Rs at T=95%
    r_t95= Z0/((((0.95**(-1/2))-1)*p)**(n+1))
    print(r_t95)
    Rs_T95.append(r_t95)
    T95.append(0.95)
    
   
plt.plot(Rs_T95,T95,color='k',marker='o',linestyle='None',label='Rs at T=95%, percolative fit')    
plt.title('T vs Rs')
plt.ylabel('T')
...
#function to fit to test Rs ~wire_length^-1.5
def Rs_l_fit(l,a,b):
    return (a*l**(b))

min_res_list=[]

#Plot Minimum Rs (fixed network density, max T) vs varied parameter
for i in np.arange(0,j+1, 1):
    plotcolor=colors[i]
    ...
    plt.plot(varied_param[i], resistance_srt[0],plotcolor, marker='o', linestyle='-',label=  data_label[i])
    
#    # excluded for now, not very useful
...
# Tried to fit curve for Rs vs wire_length. Code below didn't fit properly, feel free to edit or omit!
'''
print(min_res_list)
...
plt.title('Minimum Rs vs ' + varied_param_title)
plt.xlabel(varied_param_axis)
plt.ylabel('Rs (Ohm)')
plt.legend(loc='upper left', bbox_to_anchor=(1.0, 1.0))
plt.tight_layout()
plt.show()
...
#Plot Rs at T=95% vs varied parameter
for i in np.arange(0,j+1, 1):
    plotcolor=colors[i]
   

    plt.plot(varied_param[i], Rs_T95[i],plotcolor, marker='o', linestyle='-',label=  data_label[i])

plt.title('Rs at T=95% vs ' + varied_param_title)
plt.xlabel(varied_param_axis)
plt.ylabel('Rs (Ohm/sq)')
plt.legend(loc='upper left', bbox_to_anchor=(1.0, 1.0))
plt.tight_layout()
plt.show()

#function to find Jcn Dens at Rs at T=95%
def jcn_rs_func(r,a,b):
    return a/(r**b)
jcn_Rs_T95=[]
#Plot Jcn Density vs Rs
for i in np.arange(0,j+1, 1):
    plotcolor=colors[i]
    popt_jcn, pcov_jcn = curve_fit(jcn_rs_func, resistancelist[i], jcn_dens_list[i])
    popt_jcn
    ...
    plt.plot(resfitlist, jcn_rs_func(resfitlist, *popt_jcn), plotcolor, linestyle='--', label='a/(r^b) fit: a=%5.3f, b=%5.3f' % tuple(popt_jcn))
    plt.plot(resistancelist[i], jcn_dens_list[i],plotcolor, marker='o', linestyle='None',label=  data_label[i])
    r=Rs_T95[i]
    a=popt_jcn[0]
    b=popt_jcn[1]
    jcn_r_t95= a/(r**b)
    jcn_Rs_T95.append(jcn_r_t95)

plt.plot(Rs_T95,jcn_Rs_T95,color='k',marker='o',linestyle='None',label='JcnDens at Rs at T=95%, percolative fit')    
plt.title('Junction Density vs Resistance')
...
#Plot Jcn Density at Rs at T=95% vs varied parameter
for i in np.arange(0,j+1, 1):
    plotcolor=colors[i]

    plt.plot(varied_param[i], jcn_Rs_T95[i],plotcolor, marker='o', linestyle='-',label=  data_label[i])

plt.title('Junction Density at T=95% vs ' + varied_param_title)
plt.xlabel(varied_param_axis)
plt.ylabel('Junction Density (junctions/um^2)')
plt.legend(loc='upper left', bbox_to_anchor=(1.0, 1.0))
plt.tight_layout()
...
#Plot max Jcn density vs varied parameter
for i in np.arange(0,j+1, 1):
    plotcolor=colors[i]
    #find endpoint
    jcnlist_len=len(jcn_dens_list[i])

    plt.plot(varied_param[i], jcn_dens_list[i][jcnlist_len-1],plotcolor, marker='o', linestyle='-',label=  data_label[i])

plt.title('Maximum Junction Density vs ' + varied_param_title)
plt.xlabel(varied_param_axis)
plt.ylabel('Junction Density (junctions/um^2)')
plt.legend(loc='upper left', bbox_to_anchor=(1.0, 1.0))
plt.tight_layout()
...
#Convert T  to T^(-1/2)-1 and take log of this array and the Rs array
for i in np.arange(0,j+1,1):
