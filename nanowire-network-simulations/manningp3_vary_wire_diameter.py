...
rho0 = 0.0785    # 0.314   #0.8   #0.0790 #0.0226   
...
n_final =  70*n_min #1.90079+31*nstep  
...
# ---------- Output file -------------  
res_file = "output_vary_diameter_1to5nm_1nmstep_l1um_bl5um_test.txt" 
...
for wire_diameter in np.arange(1, 4, 2):
  ...
  for density in np.arange(n_initial,n_final,nstep):
        ...
        while True:
          try:     
            ...
            if distl == True:  
                ...
                length_array = lengths.rvs(size=int(nwires))
            else:  
                length_array.fill(wire_length)  

            n, bins, patches = plt.hist(length_array, 50, density=True, facecolor='g', alpha=0.75)


            plt.xlabel('Tube Length')
            plt.ylabel('Counts')
            plt.title('Histogram of lengths')
            plt.text(60, .025, r'$\mu=1,\ \sigma=0.2$')
            plt.grid(True)
            plt.close()
      ...
      if np.isnan(transmittance) or np.isnan(resAvg) != True:
           ...
           resistance_stdev_list[d_counter].append(resStd)
           network_dens_list[d_counter].append(density)
           jcn_dens_list[d_counter].append(junc_avg)
           AF_list[d_counter].append(round(AF,9))
...
for j in np.arange(0,d_counter,1):
    ...
    network_dens_list[j]=np.array(network_dens_list[j], dtype=np.float64)
    jcn_dens_list[j]=np.array(jcn_dens_list[j], dtype=np.float64)
...
#Plot StDev of Resistance vs resistance
for i in np.arange(0,d_counter, 1):
    plotcolor=colors[i]
    plt.plot(resistancelist[i], resistance_stdev_list[i], plotcolor, marker='o', linestyle='-',label='diameter=%s nm'%(diameterlist[i]))
...
for i in np.arange(0,d_counter,1):
    ...
    plotcolor=colors[i]
    plt.plot(network_dens_list[i], ratio_list, plotcolor,marker='o', linestyle='-', label='diameter=%s nm'%(diameterlist[i]))
...
for i in np.arange(0,d_counter, 1):
    plotcolor=colors[i]
    plt.plot(network_dens_list[i], jcn_dens_list[i],plotcolor, marker='o', linestyle='-',label='diameter=%s nm'%(diameterlist[i]))
