...
# ---------- Output file -------------  
res_file = "output_vary_box_length_3_to_13um_2um_step_l1um_uniforml_d1nm_Rjcn1_Rwire100k__test.txt" 
...
for i in np.arange(0,l_counter,1):
    ...
    plt.plot(resistancelist[i], transmittancelist[i], plotcolor, marker= 'o', linestyle='None', label='box length=%s um' %(lengthlist[i]))
     
plt.title('T vs Rs, Rwire= %s \u03A9, Rjcn= %s \u03A9' %(wire_res, R_junc))
plt.ylabel('T')
...
for i in np.arange(0,l_counter, 1):
    plotcolor=colors[i]
    plt.plot(resistancelist[i], resistance_stdev_list[i], plotcolor, marker='o', linestyle='None', label='box length=%s um' %(lengthlist[i]))
...
for i in np.arange(0,l_counter,1):
    ...
    #print(ratio_list)
    plotcolor=colors[i]
    plt.plot(network_dens_list[i], ratio_list, plotcolor,marker='o', linestyle='-', label='box length=%s um'%(lengthlist[i]))

plt.title('St.Dev.Rs/Rs vs Network Density, Rwire= %s \u03A9, Rjcn= %s \u03A9' %(wire_res, R_junc))
...
for i in np.arange(0,l_counter, 1):
    plotcolor=colors[i]
    plt.plot(network_dens_list[i], jcn_dens_list[i],plotcolor, marker='o', linestyle='-',label='length=%s um' %(lengthlist[i]))

plt.title('Junction Density vs Network Density, Rwire= %s \u03A9, Rjcn= %s \u03A9' %(wire_res, R_junc))
plt.xlabel('Network Density (wires/um^2)')
...
plt.title('Log(T^(-1/2)-1) vs Log(Rs) with Line Fit, Rwire= %s \u03A9, Rjcn= %s \u03A9' %(wire_res, R_junc))
