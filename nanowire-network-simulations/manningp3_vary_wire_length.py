...
n_final =  120*n_min #1.90079+31*nstep  
...
# ---------- Output file -------------  
res_file = "output_vary_wire_lengths_0.8to1.2um_0.1um_steps_d1nm_bl10um_uniforml_Rjcn100k_Rtube100k_constAF_test.txt" 
...
#set wire length range
for wire_length in np.arange(1, 1.3, 0.2):
  ...
  rho0 = (math.pi*0.000001*(wire_diameter/2)**2)/wire_length
  ...
  #set density range for data points
  #scale density with wire length so all data points increase in steps of AF (and Transmittance)
  nstep=(10*n_min)/wire_length
  n_initial=(40*n_min)/wire_length
  n_final=(80*n_min)/wire_length
  ...
  for density in np.arange(n_initial,n_final,nstep):
      for sample in range(samples):
        while True:
          try:     
            ...
            plt.close()
            #print(type(range(2*nintersections)))
      ...
      if np.isnan(transmittance) or np.isnan(resAvg) != True:
           ...
           resistance_stdev_list[l_counter].append(resStd)
           network_dens_list[l_counter].append(density)
           jcn_dens_list[l_counter].append(junc_avg)
           AF_list[l_counter].append(round(AF,9))
...
#print(resistance_stdev_list)
#print(lengthlist)
#print(network_dens_list)
#print(jcn_dens_list)
#print(nanotube_res)
#print(Rjcn_list)
#print(AF_list)
...
for i in np.arange(0,l_counter,1):
    ...
    plt.plot(resistancelist[i], transmittancelist[i], plotcolor, marker= 'o', linestyle='None', label='length=%s um, Rwire= %s \u03A9, Rjcn= %s \u03A9' %(lengthlist[i],nanotube_res[i],Rjcn_list[i]))
...
for i in np.arange(0,l_counter, 1):
    plotcolor=colors[i]
    plt.plot(resistancelist[i], resistance_stdev_list[i], plotcolor, marker='o', linestyle='-',label='length=%s um, wire resistance= %s \u03A9' %(lengthlist[i],nanotube_res[i]))
...
for i in np.arange(0,l_counter,1):
    ratio_list=[]
    ratio_list = [k / j for k, j in zip(resistance_stdev_list[i], resistancelist[i])]
    #print(ratio_list)
    plotcolor=colors[i]
    plt.plot(network_dens_list[i], ratio_list, plotcolor,marker='o', linestyle='-', label='length=%s um, wire resistance= %s \u03A9'%(lengthlist[i],nanotube_res[i]))
...
for i in np.arange(0,l_counter, 1):
    plotcolor=colors[i]
    plt.plot(network_dens_list[i], jcn_dens_list[i],plotcolor, marker='o', linestyle='-',label='length=%s um, wire resistance= %s \u03A9' %(lengthlist[i],nanotube_res[i]))
...
#Line fit of Log(T^(-1/2)-1) vs Log(Rs)
for i in np.arange(0,l_counter,1):
    ...
    plotcolor=colors[i]
    plt.plot(resistancelist[i],transmittancelist[i], plotcolor, marker='o', linestyle='None', label='length=%s um, wire resistance= %s \u03A9' %(lengthlist[i],nanotube_res[i]))
    plt.plot(resistancelist[i], regression_line, plotcolor, linestyle= '-', label='Line Fit y = %s x + %s , l=%s um' %(round(m,3),round(b,3),lengthlist[i]))
