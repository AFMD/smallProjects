...
@author: sturdzal
"""
#Vary wire length and scale box size with wire length
...
# ---------- Output file -------------  
res_file = "output_box_length_5xwire_length_wl_from_0.8_to_1.2um_0.1um_step_d1nm_final.txt" 
if os.path.exists(res_file)==False:
    open(res_file, "w").write("Density AF Transmittance Average_resistance resStdev Resistance_per_area Junct_density R_junc rho0 wire_diameter wire_length box_length samples nstep n_initial n_final tolerance_minres distl lower_l upper_l sigmal junctions_removal calctime\n")
...
for wire_length in np.arange(1, 1.3, 0.2):
  ...
  #set box length and scaling factor
  box_length=5*wire_length
  boxlengthlist.append(box_length)
  elec_length = box_length    
  box_y = box_length  
  lead_sep = box_length
...
for j in np.arange(0,l_counter,1):
    ...
    resistance_stdev_list[j]=np.array(resistance_stdev_list[j], dtype=np.float64)
...
for i in np.arange(0,l_counter,1):
    ...
    plt.plot(resistancelist[i], transmittancelist[i], plotcolor, marker= 'o', linestyle='None', label='wire length=%s um, box length=%s um' %(lengthlist[i],boxlengthlist[i]))
...
for i in np.arange(0,l_counter, 1):
    plotcolor=colors[i]
    plt.plot(resistancelist[i], resistance_stdev_list[i], plotcolor, marker='o', linestyle='None', label='wire length=%s um, box length=%s um' %(lengthlist[i], boxlengthlist[i]))

plt.title('Rs vs St.Dev. in Rs')
plt.xlabel('Rs')
plt.ylabel('Standard Deviation in Rs')
plt.legend(loc='upper left', bbox_to_anchor=(1.0, 1.0))
plt.tight_layout()
plt.show()
...
for i in np.arange(0,l_counter,1):
    ...
    plotcolor=colors[i]
    plt.plot(resistancelist[i],transmittancelist[i], plotcolor, marker='o', linestyle='None', label='wire length=%s um, box length=%s um' %(lengthlist[i],boxlengthlist[i]))
    plt.plot(resistancelist[i], regression_line, plotcolor, linestyle= '-', label='Line Fit y = %s x + %s , wire l=%s um, box l=%s um' %(round(m,3),round(b,3),lengthlist[i], boxlengthlist[i]))
